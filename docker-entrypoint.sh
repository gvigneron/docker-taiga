...
if [ -z "$TAIGA_SKIP_DB_CHECK" ]; then
  ...
  python3 /src/taiga-back/checkdb.py
  DB_CHECK_STATUS=$?
  ...
  if [ $DB_CHECK_STATUS -eq 1 ]; then
    ...
    python3 /src/taiga-back/manage.py migrate --noinput
    python3 /src/taiga-back/manage.py loaddata initial_user
    python3 /src/taiga-back/manage.py loaddata initial_project_templates
    python3 /src/taiga-back/manage.py loaddata initial_role
    python3 /src/taiga-back/manage.py compilemessages
  fi
fi

# Look for static folder, if it does not exist, then generate it
if [ ! -d "/usr/src/taiga-back/static" ]; then
  python3 /src/taiga-back/manage.py collectstatic --noinput
fi

# Automatically replace "TAIGA_HOSTNAME" with the environment variable
sed -i "s/TAIGA_HOSTNAME/$TAIGA_HOSTNAME/g" /src/taiga-front-dist/dist/conf.json
...
if [ "$TAIGA_SSL_BY_REVERSE_PROXY" = "True" ]; then
  ...
  sed -i "s/http:\/\//https:\/\//g" /src/taiga-front-dist/dist/conf.json
  sed -i "s/ws:\/\//wss:\/\//g" /src/taiga-front-dist/dist/conf.json
...
#   mv /etc/nginx/ssl.conf /etc/nginx/conf.d/default.conf
  ...
  sed -i "s/https:\/\//http:\/\//g" /src/taiga-front-dist/dist/conf.json
  sed -i "s/wss:\/\//ws:\/\//g" /src/taiga-front-dist/dist/conf.json
fi
